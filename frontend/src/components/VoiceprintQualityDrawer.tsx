...
export function VoiceprintQualityDrawer({
  ...
  return (
    <Drawer
      ...
      title="声纹质量中心"
    >
      {/* A failed policy fetch used to render two titled but empty sections
          and drop the pipeline-status alert entirely, which reads as "no
          thresholds configured". PanelState keeps the failure visible and
          retryable instead. */}
      <PanelState
        pending={policyQuery.isLoading}
        error={policyQuery.error}
        empty={!policy}
        emptyTitle="暂无声纹策略"
        emptyDescription="后端未返回采样与合并阈值配置。"
        onRetry={() => void policyQuery.refetch()}
        pendingLabel="正在加载声纹策略…"
      >
        {policy ? (
          <>
            <Alert
              ...
              data={[
                {
                  ...
                  value: (
                    <span>
                      cos ≥ {policy.layer1.ambiguous_threshold} 直接合并；
                      {policy.layer1.cosine_threshold} –{" "}
                      {policy.layer1.ambiguous_threshold} 合并并标记{" "}
                      <Tag size="small" color="gold">AMBIGUOUS</Tag>
                      （检索降权）；低于 {policy.layer1.cosine_threshold} 新建说话人
                    </span>
                  ),
                },
                {
                  label: "Layer 2 · 名称模糊",
                  value: policy.layer2.enabled ? (
                    <span>
                      fuzzy ≥ {policy.layer2.fuzzy_inferred_threshold}{" "}
                      产生合并提议，进入{" "}
                      <Tag size="small" color="red">PENDING_REVIEW</Tag>
                      （检索隐藏）；声纹余弦 ≥{" "}
                      {policy.layer2.voiceprint_reconfirm_cosine}{" "}
                      可自动核定，否则等待人工复核
                    </span>
                  ) : (
      ...
      <Tabs defaultActiveTab="pending">
        <Tabs.TabPane
          key="pending"
          // A failed count must not be spelled "0": the queue exists to catch
          // merges that should not have happened, so an unknown count reads
          // as "—" and the panel below says why.
          title={
            pendingQuery.error ? "待复核（—）" : `待复核（${pendingTotal}）`
          }
        >
          {/* PanelState, not a bare empty branch: a rejected request rendered
              as 无待处理项 tells the reviewer there is nothing to review. */}
          <PanelState
            pending={pendingQuery.isLoading}
            error={pendingQuery.error}
            empty={pendingTotal === 0}
            emptyTitle="无待处理项"
            emptyDescription="当前没有等待人工复核的合并提议。"
        ...
        <Tabs.TabPane
          key="resolved"
          title={
            resolvedQuery.error ? "已处理（—）" : `已处理（${resolvedTotal}）`
          }
        >
          <PanelState
            pending={resolvedQuery.isLoading}
            error={resolvedQuery.error}
            empty={resolvedTotal === 0}
            emptyTitle="暂无历史记录"
            emptyDescription="确认或驳回过的合并提议会出现在这里。"
            onRetry={() => void resolvedQuery.refetch()}
            pendingLabel="正在加载历史记录…"
          >
            <Table
              data={resolvedRows}
              rowKey="id"
