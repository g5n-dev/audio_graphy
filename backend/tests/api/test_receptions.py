    ...
    async def assemble(
        ...
        inputs = tuple(
            AudioInputManifest(
                ...
                size_bytes=100,
                duration_sec=10.0,
                timeline_start_sec=index * 10.0,
                timeline_end_sec=(index + 1) * 10.0,
                codec="pcm_s16le",
                sample_rate=16_000,
                channels=1,
            )
        ...
        return AudioAssemblyManifest(
            ...
            total_duration_sec=sum(item.duration_sec for item in inputs),
...
class TestReceptionCreateAndRead:
    ...
    def test_physical_merge_rejects_clipped_source_mapping_explicitly(
        self,
        test_client: TestClient,
        auth_headers: dict[str, dict[str, str]],
        db_session_factory: Any,
        ...
        body = _create_body([recording_id])
        body["merge_mode"] = "logical"
        body["recordings"][0]["source_start_sec"] = 1.0
        body["recordings"][0]["source_end_sec"] = 11.0
        created = test_client.post(
            "/api/v1/receptions",
            json=body,
            headers=auth_headers["admin_t1"],
        ...
        assert response.status_code == 422
        assert response.json()["error"]["code"] == "AUDIO_CLIP_ASSEMBLY_UNSUPPORTED"
