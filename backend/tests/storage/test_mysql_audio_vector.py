...
class TestBruteCosineInMemory:
    ...
    def test_top_k_sorted_descending(self) -> None:
        ...
        rows = [(i, MySQLAudioVectorStore._serialize(self._make_unit_vec(7 + i))) for i in range(5)]
    ...
    def test_dim_mismatch_row_skipped(self) -> None:
        ...
        hits = store._brute_cosine([(1, wrong_blob), (2, good_blob)], query, top_k=5)
        assert len(hits) == 1
        assert hits[0].id == 2
    ...
    def test_corrupt_blob_skipped(self) -> None:
        ...
        hits = store._brute_cosine([(1, b"\x00\x00\x00"), (2, good_blob)], query, top_k=5)
        assert len(hits) == 1
        assert hits[0].id == 2
...
class TestAudioVectorStoreDB:
    ...
    async def test_tenant_isolation(
        ...
        await audio_store.upsert_audio_vector("tenant_a", rec_a, 400, vec, chunk_id=401)
        await audio_store.upsert_audio_vector("tenant_b", rec_b, 401, vec, chunk_id=402)

        hits_a = await audio_store.search_audio("tenant_a", vec, top_k=10)
        hits_b = await audio_store.search_audio("tenant_b", vec, top_k=10)
        assert {h.segment_id for h in hits_a} == {400}
        assert {h.segment_id for h in hits_b} == {401}
    ...
    async def test_get_audio_vectors_for_recording(
        ...
        await audio_store.upsert_audio_vector("default", rec_id, 500, vec1, chunk_id=11)
        await audio_store.upsert_audio_vector("default", rec_id, 501, vec2, chunk_id=None)

        rows = await audio_store.get_audio_vectors_for_recording("default", rec_id)
        assert len(rows) == 2
    ...
    async def test_get_audio_vectors_for_recording_cross_tenant_empty(
        ...
        await audio_store.upsert_audio_vector("tenant_a", rec_id, 600, vec, chunk_id=601)
        # Wrong tenant → empty list
        rows = await audio_store.get_audio_vectors_for_recording("tenant_b", rec_id)
        assert rows == []
