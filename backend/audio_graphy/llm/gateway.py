...
class LLMPriceSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isinstance(rate, int) or isinstance(rate, bool) or rate < 0 for rate in rates):
...
class LLMUsageContext:
    ...
    def __post_init__(self) -> None:
        ...
        if self.logical_request_id is not None and not (1 <= len(self.logical_request_id) <= 64):
...
class LLMGateway:
    ...
    async def execute(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            if shadow_recipe_sha256 is not None:
                ...
                if not shadow_cache_hit and self._response_is_valid(request, response):
    ...
    async def _validated_v1_cache_response(
        ...
        if not self._response_is_valid(request, response):
            logger.warning(
                "Legacy v1 LLM cached response failed validation; evicting namespace=%s hash=%s",
    ...
    def _serving_recipe_version(self, request: LLMRequest) -> RecipeVersion:
        if self._recipe_migration_mode == "shadow" and request.recipe_version == _RECIPE_VERSION_V2:
            return _RECIPE_VERSION_V1
        return request.recipe_version

    def _shadow_recipe_sha256(self, request: LLMRequest) -> str | None:
        if self._recipe_migration_mode != "shadow" or request.recipe_version != _RECIPE_VERSION_V2:
            return None
    ...
    async def _call_provider_with_retry(
        ...
        for attempt in range(self._max_retries + 1):
            ...
            response = replace(
                ...
                price_version=(
                    self._price_snapshot.version if self._price_snapshot is not None else None
                ),
    ...
    while True:
        try:
            ...
            rejected_keyword = next(
                (key for key in tuple(kwargs) if _is_unexpected_keyword_error(exc, key)),
                None,
            )
